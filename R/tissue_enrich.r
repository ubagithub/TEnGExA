...
TEnGExA.default <- function(x, fpkm_flag=0, threshold=5, tissue_num= 5, min_fpkm= 1, ...) {
  ...
  for (i in 1:r1)
  {
    ...
    fpkm_row_vals <- unlist(fpkm[i, u:c1])          
    maxfpkm <- sort(fpkm_row_vals, decreasing = TRUE) 
    ...
    for (j in 1:(t1-1))
    {
      ...
      {
        fpkm[i,c1+2*j]=names(maxfpkm)[j]  
        if (j<=1)
        {flageg=1
        egstr<-paste("Tissue-Enriched:" ,names(maxfpkm)[j])       
        }
        else
        {   if (flageg==0)
    ...
    fpkm[i,c1+2*(t1-1)+2]=names(maxfpkm)[1]      
    
    if (fpkm[i,c1+2*(t1-1)+1]>=threshold  & flageg==0 & maxfpkm[1]>=min_fpkm )
    { egstr<-paste("Tissue-Enhanced:" ,names(maxfpkm)[1])      
    flageg=1}
